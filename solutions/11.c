...
int main() {
  int grid[20][20] = {
      ...
      {1,  70, 54, 71, 83, 51, 54, 69, 16, 92,
       33, 48, 61, 43, 52, 0,  89, 19, 67, 48}};

  int currentMax = 0;
  for (int rowIndex = 0; rowIndex < 17; rowIndex++) {
    for (int colIndex = 0; colIndex < 17; colIndex++) {
      const int initialNum = grid[rowIndex][colIndex];

      // Check east
      if (grid[rowIndex][colIndex + 1] && grid[rowIndex][colIndex + 2] &&
          grid[rowIndex][colIndex + 3]) {
        const int east1 = grid[rowIndex][colIndex + 1];
        const int east2 = grid[rowIndex][colIndex + 2];
        const int east3 = grid[rowIndex][colIndex + 3];

        const int product = initialNum * east1 * east2 * east3;
      ...
      if (grid[rowIndex + 1][colIndex + 1] &&
          grid[rowIndex + 2][colIndex + 2] &&
          grid[rowIndex + 3][colIndex + 3]) {
        const int southeast1 = grid[rowIndex + 1][colIndex + 1];
        const int southeast2 = grid[rowIndex + 2][colIndex + 2];
        const int southeast3 = grid[rowIndex + 3][colIndex + 3];

        const int product = initialNum * southeast1 * southeast2 * southeast3;
        if (product > currentMax) {
          currentMax = product;
        }
      }
      ...
      // Check south
      if (grid[rowIndex + 1][colIndex] && grid[rowIndex + 2][colIndex] &&
          grid[rowIndex + 3][colIndex]) {
        const int south1 = grid[rowIndex + 1][colIndex];
        const int south2 = grid[rowIndex + 2][colIndex];
        const int south3 = grid[rowIndex + 3][colIndex];

        const int product = initialNum * south1 * south2 * south3;
        if (product > currentMax) {
          currentMax = product;
        }
      }
      ...
      if (grid[rowIndex + 1][colIndex - 1] &&
          grid[rowIndex + 2][colIndex - 2] &&
          grid[rowIndex + 3][colIndex - 3]) {
        const int southwest1 = grid[rowIndex + 1][colIndex - 1];
        const int southwest2 = grid[rowIndex + 2][colIndex - 2];
        const int southwest3 = grid[rowIndex + 3][colIndex - 3];

        const int product = initialNum * southwest1 * southwest2 * southwest3;
